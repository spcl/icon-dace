...
srcdir:= /home/primrose/Work/IconGrounds/icon-dace2/icon-model
...
LDFLAGS= -L/usr/lib/x86_64-linux-gnu/  -Wl,-rpath -Wl,/usr/lib/x86_64-linux-gnu/
...
LIBS= -lnetcdff -lnetcdf -lopenblas 
...
externals/ecrad/Makefile: icon.mk
	$(silent_CONFIG)$(MKDIR_P) $(@D) && cd $(@D) && '/home/primrose/Work/IconGrounds/icon-dace2/icon-model/externals/ecrad/configure'  'CC=mpicc' 'CPPFLAGS=-I/usr/include -I/usr/include/libxml2' 'CXX=g++' 'CXXFLAGS=-g -march=native' 'FC=mpif90' 'LDFLAGS=-L/usr/lib64 -L/usr/lib' 'LIBS=-lxml2 -lfyaml -leccodes -llapack -lblas -lnetcdff -lnetcdf -lstdc++' 'MPI_LAUNCH=mpiexec' '--enable-openmp' '--enable-grib2' '--enable-mixed-precision' '--enable-rte-rrtmgp' 'CC=gcc' 'FC=gfortran' 'LDFLAGS=-L/usr/lib/x86_64-linux-gnu/ ' 'LIBS=-lnetcdff -lnetcdf -lopenblas ' '--enable-acm-license' '--disable-mixed-precision' '--disable-edmf' '--disable-les' '--disable-ocean' '--disable-jsbach' '--disable-coupling' '--disable-aes' '--disable-rte-rrtmgp' '--enable-ecrad' '--disable-mpi' '--disable-mpi-checks' '--disable-openmp' '--disable-loop-exchange' '--enable-dace-subst=no' '--enable-explicit-fpp' '--disable-cdi-pio' '--disable-grib2' '--disable-option-checking' '--prefix=/usr/local' '--cache-file=/dev/null' '--srcdir=/home/primrose/Work/IconGrounds/icon-dace2/icon-model/externals/ecrad' 'FCFLAGS=-g -O2 -I/usr/include/ -Wall -frecursive -Wno-unused-variable -Wno-unused-dummy-argument -Wno-unused-function -Wno-missing-include-dirs -DDACE_SUBST_VERIFY -DDACE_SUBST_ENABLE -std=legacy' 'NETCDF_FCFLAGS=' 'NETCDF_FCLIBS=' '--enable-silent-rules=yes' '--enable-pgi-inlib=no' $(if $(silent_CONFIG),--silent)
...
externals/math-interpolation/build/Makefile: icon.mk
	$(silent_CMAKE)$(MKDIR_P) $(@D) && cd $(@D) && "${CMAKE}"  '-Wno-dev' '--no-warn-unused-cli' '-GUnix Makefiles' '-DCMAKE_EXE_LINKER_FLAGS=-L/usr/lib/x86_64-linux-gnu/ ' '-DCMAKE_Fortran_STANDARD_LIBRARIES=-lnetcdff -lnetcdf -lopenblas ' '-DCMAKE_EXE_LINKER_FLAGS=' '-DCMAKE_Fortran_STANDARD_LIBRARIES=' '-DCMAKE_INSTALL_PREFIX=/usr/local' '/home/primrose/Work/IconGrounds/icon-dace2/icon-model/externals/math-interpolation' '-DCMAKE_Fortran_FLAGS=-g -O2 -I/usr/include/ -Wall -frecursive -Wno-unused-variable -Wno-unused-dummy-argument -Wno-unused-function -Wno-missing-include-dirs -DDACE_SUBST_VERIFY -DDACE_SUBST_ENABLE -std=legacy' '-DMI_ENABLE_LOOP_EXCHANGE=no' '-DMI_ENABLE_MIXED_PRECISION=no' '-DMI_ENABLE_DIM_SWAP=no' '-DMI_ENABLE_OPENACC=OFF' '-DCMAKE_PREFIX_PATH:PATH=/home/primrose/Work/IconGrounds/icon-dace2/icon-model/build/verification/externals/fortran-support/build;/home/primrose/Work/IconGrounds/icon-dace2/icon-model/build/verification/externals/math-support/build' '-DBUILD_SHARED_LIBS:BOOL=OFF' '-DBUILD_TESTING:BOOL=OFF' '-DBUILD_STANDALONE:BOOL=OFF' '-DCMAKE_BUILD_TYPE:STRING=NOEXTRAFLAGS' $(if $(silent_CMAKE),>/dev/null)
