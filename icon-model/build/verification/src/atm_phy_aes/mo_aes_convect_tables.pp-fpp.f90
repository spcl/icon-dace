# 1 "/home/primrose/Work/IconGrounds/icon-dace2/icon-model/src/atm_phy_aes/mo_aes_convect_tables.f90"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 16 "/home/primrose/Work/IconGrounds/icon-dace2/icon-model/src/atm_phy_aes/mo_aes_convect_tables.f90" 2
...
SUBROUTINE prepare_ua_index_spline(jg, name, jcs, size, temp, idx, zalpha, &
    ...
    IF (PRESENT(xi)) THEN
      ...
      DO jl = jcs,size
        ...
        zinbounds = MERGE(zinbounds,0.0_wp,(ztt-ztmin).GE.0._wp)
...
!IBM* ASSERT(NODEPS)
    ...
    DO nl = jcs, kidx
      ...
      zinbounds = MERGE(zinbounds,0.0_wp,(ztt-ztmin).GE.0._wp) !Reversed to avoid ztt=NaN to default to last FSEL argument!
      zinbounds = MERGE(0.0_wp,zinbounds,(ztt-ztmax).GE.0._wp)
    END DO
    !$ACC END PARALLEL
