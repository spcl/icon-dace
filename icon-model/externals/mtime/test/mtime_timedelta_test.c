  ...
  for (i = 0; i < 14; i++)
    {
      ...
      switch (i % 12)
        ...
        case 7:
        case 9:
        case 11: ck_assert_int_eq(31, cur_backshift->date.day); break;
        case 1: ck_assert_int_eq(28, cur_backshift->date.day); break;
        case 3:
        case 5:
        case 8:
        case 10: ck_assert_int_eq(30, cur_backshift->date.day); break;
        default: printf(" error - this should never happen\n"); break;
        }
    }
}
END_TEST
...
START_TEST(test_event_at_end_of_month)
{
  ...
  struct _event *out
      = newEvent("outputEvent", "2013-01-01T00:00:00.000", "2013-01-01T00:00:00.000", "2019-01-01T00:00:00.000", "P1M", NULL);
  struct _event *outAtEnd
      = newEvent("outputEventAtEnd", "2012-12-31T23:50:00.000", "2013-01-01T00:00:00.000", "2019-01-01T00:00:00.000", "P1M", NULL);
...
void
add_mtime_timedelta_test_to_suite(Suite *suite)
{
...
void
assertTimeDelta(const char *input_string, bool std_form, char sign, int64_t year, int month, int day, int hour, int minute,
                int second, int ms, const char *expected_output_string)
{
...
void
assertInvalidTimeDelta(const char *input_string)
{
  TimeDelta timedelta = newTimeDelta(input_string);
...
void
assertTimeDeltaToJulianDelta(const char *timedelta_string, const char *base_datetime_string, char expected_jd_sign,
                             int64_t expected_jd_day, int64_t expected_jd_ms)
{
  ...
  size_t length = snprintf(NULL, 0, format, base_datetime_string, timedelta_string, expected_jd_sign, expected_jd_day,
                           expected_jd_ms, juliandelta->sign, juliandelta->day, juliandelta->ms)
                  + 1;
  char *msg = malloc(length);
  snprintf(msg, length, format, base_datetime_string, timedelta_string, expected_jd_sign, expected_jd_day, expected_jd_ms,
           juliandelta->sign, juliandelta->day, juliandelta->ms);
...
void
assertTimeDeltaToJulianDeltaWithoutExpectations(const char *timedelta_string, const char *base_datetime_string,
                                                char expected_jd_sign)
{
  TimeDelta timedelta = newTimeDelta(timedelta_string);
  ck_assert(timedelta != NULL);
...
void
assertGetTimeDeltaFromDateTime(const char *dt1_string, const char *dt2_string, const char *expected_td_string)
{
  ...
  ck_assert_msg(compareTimeDelta(&td, expected_td) == equal_to, "base datetime %s expected %s != %s returned", dt2_string,
                expected_td_string, str);
...
void
assertTimeDeltaToJulianDeltaToTimeDelta(const char *base_dt_string, const char *td_string)
{
  void *err;
  ...
  ck_assert_msg(compareTimeDelta(td, &res_td) == equal_to, "base datetime %s original %s != %s returned", base_dt_string, td_string,
                str);
