...
module mo_solve_nh_predictor_post_bindings
  ...
  use mo_run_config, only: &
    timers_level
  use mo_timer, only: &
    timer_solve_nh_edgecomp, &
    timer_solve_nh_vimpl
  ...
  private
  public :: run_solve_nh_predictor_post
  public :: run_solve_nh_predictor_post_verification
  public :: verify_solve_nh_predictor_post
  public :: dace_init_solve_nh_predictor_post
  public :: dace_exit_solve_nh_predictor_post
  public :: dace_program_solve_nh_predictor_post
...
interface
  ...
  type(c_ptr) function dace_init_solve_nh_predictor_post( &
    bdy_divdamp, &
    enh_divdamp_fac, &
    global_data, &
    p_int, &
    ...
    bind(c, name="__dace_init_solve_nh_predictor_post")
    use iso_c_binding
  ...
  end function dace_init_solve_nh_predictor_post

  integer(c_int) function dace_exit_solve_nh_predictor_post(state) &
    bind(c, name="__dace_exit_solve_nh_predictor_post")
    use iso_c_binding

    type(c_ptr), value :: state
  end function dace_exit_solve_nh_predictor_post

  subroutine dace_program_solve_nh_predictor_post( &
    state, &
    bdy_divdamp, &
    enh_divdamp_fac, &
    global_data, &
    ...
    bind(c, name="__program_solve_nh_predictor_post")
    use iso_c_binding
    ...
    real(kind=c_double), value :: z_w_backtraj
    real(kind=c_double), value :: zf
  end subroutine dace_program_solve_nh_predictor_post
...
contains
  ...
  function copy_in_global_data_type(steal_arrays, minimal_structs) result(dace_obj_ptr)
    ...
    if (10 /= size(ndyn_substeps_var, dim=1)) then
      print *, &
        "Array size conflicts with config propagation for array 'global_data.ndyn_substeps_var'"//char(10), &
        "    - actual = (", &
        size(ndyn_substeps_var, dim=1), &
        "), config propagated = (10)"
    end if
#endif
    dace_rich_obj%divdamp_type = divdamp_type
...
#endif
    dace_rich_obj%kstart_moist = copy_in_int32_1d_array( &
    fortran_array=logical_fix_1d(kstart_moist), &
    steal_arrays=steal_arrays, &
    use_openacc=.false., &
    minimal_structs=minimal_structs &
  )
#if defined(DACE_SUBST_VERIFY)
    if (10 /= size(kstart_moist, dim=1)) then
      print *, &
        "Array size conflicts with config propagation for array 'global_data.kstart_moist'"//char(10), &
        "    - actual = (", &
        size(kstart_moist, dim=1), &
        "), config propagated = (10)"
    end if
#endif
    dace_rich_obj%nproma = nproma
    dace_rich_obj%timers_level = timers_level
    dace_rich_obj%timer_solve_nh_edgecomp = timer_solve_nh_edgecomp
    dace_rich_obj%timer_solve_nh_vimpl = timer_solve_nh_vimpl
  ...
  subroutine copy_back_global_data_type(dace_obj_ptr)
    ...
    divdamp_type = transfer(dace_rich_obj%divdamp_type, mold=divdamp_type)
    ...
    timer_solve_nh_edgecomp = transfer(dace_rich_obj%timer_solve_nh_edgecomp, mold=timer_solve_nh_edgecomp)
    timer_solve_nh_vimpl = transfer(dace_rich_obj%timer_solve_nh_vimpl, mold=timer_solve_nh_vimpl)
  ...
  subroutine compare_global_data_type_struct( &
    ...
    call compare_int32_scalar( &
      actual=actual_rich%divdamp_type, &
      ref=transfer(divdamp_type, mold=int(1, kind=4)), &
      result=local_result, &
      scalar_expr=member_expr &
    )
    ...
    call compare_int32_1d_array( &
        actual=actual_rich%kstart_moist, &
        ref=logical_fix_1d(kstart_moist), &
        result=local_result, &
        use_openacc=.false., &
        array_expr=member_expr &
    )
    ...
    call compare_int32_scalar( &
      actual=actual_rich%timer_solve_nh_edgecomp, &
      ref=transfer(timer_solve_nh_edgecomp, mold=int(1, kind=4)), &
      result=local_result, &
      scalar_expr=member_expr &
    )
    ...
    call compare_int32_scalar( &
      actual=actual_rich%timer_solve_nh_vimpl, &
      ref=transfer(timer_solve_nh_vimpl, mold=int(1, kind=4)), &
      result=local_result, &
      scalar_expr=member_expr &
    )
  ...
  subroutine run_solve_nh_predictor_post( &
    bdy_divdamp, &
    enh_divdamp_fac, &
    p_int, &
    p_nh, &
...
real :: t0, t1
    ...
    if (is_initialized .eqv. .false.) then
      is_initialized = .true.
      dace_state = dace_init_solve_nh_predictor_post( &
...
print *, 'PREPOST INTERNAL (s): ', (t1-t0)
  ...
  end subroutine run_solve_nh_predictor_post

  subroutine run_solve_nh_predictor_post_verification( &
    bdy_divdamp, &
    enh_divdamp_fac, &
    p_int, &
    p_nh, &
  ...
  end subroutine run_solve_nh_predictor_post_verification

  subroutine verify_solve_nh_predictor_post( &
    bdy_divdamp, &
    enh_divdamp_fac, &
    p_int, &
    p_nh, &
    ...
    if (is_initialized .eqv. .false.) then
      print *, "verify_solve_nh_predictor_post: dace state is not initialized"
    end if
    ...
    if (.not. result) then
      print *, "verify_solve_nh_predictor_post: Failed verification"
    else
      print *, "verify_solve_nh_predictor_post: Verification successful :)"
    end if

  end subroutine verify_solve_nh_predictor_post

end module mo_solve_nh_predictor_post_bindings
