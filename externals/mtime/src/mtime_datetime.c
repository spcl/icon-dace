...
datetimeToString(struct _datetime* dt, char* toStr)
{
  if ((dt != NULL )&& (toStr != NULL)){
    ...
    snprintf(toStr,MAX_DATETIME_STR_LEN,"%" PRIi64 "-%02d-%02dT%02d:%02d:%02d.%03d",
...
datetimeToBasicString(struct _datetime* dt, char* toStr)
{
  if ((dt != NULL )&& (toStr != NULL)){
    ...
    if (dt->date.year < 0)
      snprintf(toStr,MAX_DATETIME_STR_LEN,"% 05d%02d%02dT%02d%02d%02d.%03d",
                        (int)dt->date.year, dt->date.month, dt->date.day,
                        dt->time.hour, dt->time.minute, dt->time.second, dt->time.ms); //TODO on LUIS: Should we block when year is > 9999 format?
    else
      snprintf(toStr,MAX_DATETIME_STR_LEN,"%04d%02d%02dT%02d%02d%02d.%03d",
                        (int)dt->date.year, dt->date.month, dt->date.day,
                        dt->time.hour, dt->time.minute, dt->time.second, dt->time.ms);
...
datetimeToPosixString(struct _datetime* dt, char* toStr, char* fmtString)
{
  if ((dt != NULL ) && (toStr != NULL) && (fmtString != NULL))
  {
    ...
    memset(toStr,'\0',MAX_DATETIME_STR_LEN);
    strftime(toStr, MAX_DATETIME_STR_LEN, fmtString, &tm_info); /*CRITICAL: Careful what you wish for. Incompatiable fmtString will cause core dump*/
