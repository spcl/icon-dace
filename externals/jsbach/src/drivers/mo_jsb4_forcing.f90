...
MODULE mo_jsb4_forcing
  ...
  USE mo_physical_constants,     ONLY: tmelt, p0sl_bg, rd, amco2, amd, cpd, vtmpc1, vtmpc2, rdv
  USE mo_jsb_physical_constants, ONLY: cvdifts, grav ! at the end grav comes from mo_physical_constants,
                                                     ! however like this it is consistent with JSBACH
  USE mo_orbit_solar,            ONLY: inquire_declination

  USE mo_jsb_base,               ONLY: jsbach
  ...
  USE mo_jsb_io_netcdf_iface,    ONLY:           &
    ...
    & NF_MAX_NAME, nf_get_att_text, NF_NOERR       ! from netcdf.inc
...
CONTAINS
  ...
  !>
  !> provides the variables required for jsb4 interface calls in this timestep based on (namelist defined) external forcing
  !!
    ...
    CASE(RH_)
      ...
      DO iblk = 1, nblks
        ...
        ELSE
          ic = ice
        END IF
        CALL sat_specific_humidity(t_air(1:ic,iblk), press_srf(1:ic,iblk), sat_spec_humidity(1:ic,iblk))
      ENDDO
  ...
  !>
  !! init reading forcing data (read namelist, set options...)
  !!
  SUBROUTINE init_forcing(model_id, dtime, steplen)
    ...
    NAMELIST /jsb_forcing_nml/ cyclic_nyears, cyclic_start_year, forcing_synchron_factor, forcing_steps_per_day,              &
      & forcing_set_ocean_to_constants, forcing_height_wind, forcing_height_humidity,                                         &
...
!            & TRIM(forcing_input_variable%expectedUnit), readUnit)
  ...
  !>
  !! gets the length and the content of the time dimension
  !!
  ...
  !>
  !! calculates the remaining input required for jsbach4
  !!
  SUBROUTINE get_remaining_input_for_jsb4_interface(nblks, ice, icz, temp_air, pressure, qair, wind, &
    ...
    INTEGER  :: ic, iblk
    REAL(wp) :: zcons9, zcons11, zcons12, zkappa, zsigma
    ...
    DO iblk = 1,nblks
      ...
      ztvd(1:ic) = ( temp_air(1:ic, iblk) * ( 100000._wp/air_pressure(1:ic))**zkappa ) * ( 1._wp + vtmpc1 * qair(1:ic, iblk))
      ztvir(1:ic) = temp_air(1:ic, iblk) * ( 1._wp + vtmpc1 * qair(1:ic, iblk) )

      ! virtual potential surface temperature
      CALL sat_specific_humidity(t_srf_proc(1:ic,iblk), pressure(1:ic,iblk), qsat_surf(1:ic))
      ztvs(1:ic) = t_srf_proc(1:ic,iblk) * ( 100000._wp/pressure(1:ic,iblk))**zkappa * &
        & ( 1._wp + vtmpc1 * ( fact_qsat_srf_proc(1:ic,iblk) * qsat_surf(1:ic) + ( 1._wp - fact_q_air_proc(1:ic,iblk) ) &
        & * qair(1:ic,iblk)))
      ...
      zchnl(1:ic) = ckap**2 / (LOG(1._wp + zg(1:ic) / (grav * rough_m_srf_proc(1:ic,iblk) )) &
      ...
      zcons(1:ic) = zcons12 * pressure(1:ic,iblk) / ( temp_air(1:ic,iblk) * (1._wp + vtmpc1 * qair(1:ic,iblk)))
      ...
      t_bcoef(1:ic,iblk) = cpd * (1 + vtmpc2 * qair(1:ic,iblk)) * temp_air(1:ic,iblk) + zgh(1:ic)
  ...
  SUBROUTINE spec_humidity_from_rel_humidity(ice, sinlat, coslat, rel_humidity, Tmin, Tmax, elevation, qair)
    ...
    DO i = 1,ice
      ...
      CALL sat_specific_humidity(tmp(:) + tmelt, pressure(:), qsat(:)) ! present saturated qair
  ...
  FUNCTION vapour_pressure_from_evapor(nblks, ice, icz, evapo_ratio_mean, Tmin, Tair)
    ...
    DO j = 1,nblks
      ...
      DO i = 1,ic
        ...
        IF (Tmin(i,j) + tmelt < 50._wp .OR. Tmin(i,j) + tmelt > 400._wp) THEN
          WRITE (message_text,*) &
            & 'tmin temperature out of range: ', Tmin(i,j), ' degC. One thing to check: consistency of land-sea mask and forcing.'
          CALL finish(TRIM(routine), TRIM(message_text))
        ENDIF
        IF (Tair(i,j) + tmelt < 50._wp .OR. Tair(i,j) + tmelt > 400._wp) THEN
          WRITE (message_text,*) &
            & 'air temperature out of bounds: ', Tair(i,j), ' degC. One thing to check: consistency of land-sea mask and forcing.'
          CALL finish(TRIM(routine), TRIM(message_text))
        ENDIF
